...
from gaussian_splatting.utils.camera_utils import get_uniform_poses
import numpy as np
import json
...
def constructVideoNVSInfo(
    ...
    n_test_poses = min(len(images), 10)
    test_poses = get_uniform_poses(n_test_poses, radius, elevation)
    test_w2cs = np.linalg.inv(test_poses)
    test_cam_infos = []

    for idx, pose in enumerate(test_w2cs):
    ...
    ply_path = f"/tmp/{str(uuid.uuid4())}.ply"
    storePly(ply_path, xyz, SH2RGB(shs) * 255)
    pcd = fetchPly(ply_path)
