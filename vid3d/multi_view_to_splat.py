...
from gaussian_splatting.utils.loss_utils import l1_loss, ssim, lpips
...
if __name__ == "__main__":
    ...
    parser.add_argument("--multi_view_dir", type=str, required=True)
    ...
    parser.add_argument("--sub_sample_frames", type=int, default=None)
    ...
    image_dirs = set()
    for root, dirs, files in os.walk(args.multi_view_dir):
        for file in files:
    ...
    def numerical_sort(value):
        # This finds all numbers in the filename and returns them at the start of the tuple
    ...
    for image_dir in image_dirs:
        ...
        if args.sub_sample_frames:
            skip_rate = int(1 / (args.sub_sample_frames / len(images)))
            images = images[::skip_rate]

        cur_lp = lp.extract(args)
        cur_lp.images = images
        cur_lp.num_frames = len(images)

        frame_id = "/".join(image_dir.split("/")[2:])
        save_dir = os.path.join(args.out_dir, frame_id)
        cur_lp.model_path = save_dir

        training(cur_lp, op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from)
